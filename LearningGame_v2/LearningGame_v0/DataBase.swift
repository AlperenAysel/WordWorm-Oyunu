...
class DataBase {
    ...
    func ListeleTest() {
        do {
            ...
            for kelime in kelimeler {
                ...
                let gunSayısı = Zaman(eklenenTarih: kelime[self.eklemeTarihi]).aradanGecenZaman
                print(gunSayısı)
            }
        } catch  {
            print(error)
    ...
    func IngVeTr() -> [String:String] {
        ...
        do {
            ...
            for kelime in kelimeler {
                ...
                SozlukleriEkleme(left: &ingToTr, right: Zaman().bugununLİstesiniCıkar(aradanGecenGunSayısı: gunSayısı, ogrenmeSeviyesi: kelime[self.ogrenmeSeviyesi], ingKelime: kelime[self.ingKelime], trKelime: kelime[self.trKelime]))
            }
    ...
    func OgrenmeSeviyesiniArttır(ogrenilenIngKelimeId: Int) {
        ...
        let kelimeGuncelle = degisecekKelimeId.update(self.ogrenmeSeviyesi <- self.ogrenmeSeviyesi+1)
        do {
    ...
    func OgrenmeSeviyesiSıfırla(sifirlanacakKelimeId: Int) {
        ...
        let kelimeGuncelle = degisecekKelimeId.update(self.ogrenmeSeviyesi <- 1)
        do {
            try self.dataBase.run(kelimeGuncelle)
        } catch  {
            print(error)
